...
```{r, message=FALSE}
# Load climate data CSV files for Melbourne and Sydney
melb <- read_csv('Climate Data Melbourne-1.csv')
syd <- read_csv('Climate Data Sydney-1.csv')

# Keep only the relevant columns: Date (1st), Solar Exposure (2nd), MaxTemp (4th)
melb <- cbind(melb[1:2], melb[4]) 
syd <- cbind(syd[1:2], syd[4])

# Add a column to indicate the city for each dataset
...
# Reorder columns so 'City' comes first
...
# Sort the data by city name for better organization
